...
- **Kanban**: boards with configurable columns (Trello-style), cards with title/description, drag-style ordering (`order` per column), a lifecycle `status` (open/done/closed) independent of column placement, assignees (multi-user), a global tag vocabulary with colors, and embedded comments attributed to the authenticated caller (not client-supplied).
